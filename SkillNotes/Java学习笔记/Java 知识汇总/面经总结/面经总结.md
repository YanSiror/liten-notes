...
- **四次挥手**
  ...
  **为什么不是三次挥手?**
  ...
  ​	确保数据的完整发送和接受, 需要进行四次挥手来确保双方都能完成数据的传输; 通过四次挥手，可以确保双方都能明确地知道对方已经关闭了连接，并避免了不同步的情况。
  
  **为什么客户端要等待超时时间？**
  
  ​	为了保证对方已经收到ACK包，因为假设客户端发送完最后一包ACK包后释放了连接，一旦ACK包在网络中丢失，服务端将一直停留在 最后确认状态，也是为了保证在不可靠的网络链路中进行可靠的连接断开确认。在客户端发送最后的确认消息后，服务器可能还有一些延迟发送的数据包在传输中。
